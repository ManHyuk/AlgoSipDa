package problems.HackerRank;

import java.io.BufferedWriter;
import java.io.FileWriter;
import java.io.IOException;
import java.util.*;

public class sock_merchant {

    // Complete the sockMerchant function below.
    static int sockMerchant(int n, int[] ar) {

        HashMap<Integer, Integer> map = new HashMap<Integer, Integer>();

        for (int i = 0; i < n; i++) {
            if (map.containsKey(ar[i])) {
                map.put(ar[i], map.get(ar[i]) + 1);
            } else {
                map.put(ar[i], 1);
            }
        }

        int[] removedArr = removeDup(ar);


        int result = 0;
        for (int i = 0; i < removedArr.length; i++) {
//            System.out.println(map.get(removedArr[i]).intValue() / 2 );
            if ((map.get(removedArr[i]).intValue() / 2) >= 1) {
//                System.out.println(map.get(removedArr[i]));
                result += map.get(removedArr[i]).intValue() / 2;
            }
        }
//        System.out.println(result);
        return result;
    }

    public static int[] removeDup(int[] intArray) {
        if (intArray == null || intArray.length == 0) return (int[]) null;

        LinkedHashSet<Integer> hSet = new LinkedHashSet<Integer>();

        for (int i : intArray) {
            hSet.add(i);
        }

        Iterator<Integer> it = hSet.iterator();
        int[] rtnIntArr = new int[hSet.size()];
        for (int i = 0; i < hSet.size(); i++) {
            rtnIntArr[i] = it.next();
        }

        return rtnIntArr;
    }


    public static void solve() throws IOException {
        Scanner scanner = new Scanner(System.in);
//        BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(System.getenv("OUTPUT_PATH")));

        int n = scanner.nextInt();
        scanner.skip("(\r\n|[\n\r\u2028\u2029\u0085])?");

        int[] ar = new int[n];

        String[] arItems = scanner.nextLine().split(" ");
        scanner.skip("(\r\n|[\n\r\u2028\u2029\u0085])?");

        for (int i = 0; i < n; i++) {
            int arItem = Integer.parseInt(arItems[i]);
            ar[i] = arItem;
        }

        int result = sockMerchant(n, ar);

//        bufferedWriter.write(String.valueOf(result));
//        bufferedWriter.newLine();

//        bufferedWriter.close();

        scanner.close();
    }
}
